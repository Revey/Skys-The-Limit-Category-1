...
import { VctFilter } from './VctFilter'
    ...
    <nav className="fixed top-0 left-0 right-0 z-50 bg-black/80 backdrop-blur-lg border-b border-[#00aeef]/20 animate-fade-in">
      <div className="max-w-7xl mx-auto px-3 py-2 md:px-6 md:py-4">
        <div className="flex flex-wrap items-center gap-2 md:flex-nowrap">
            ...
            <div className="flex min-w-max items-center gap-1">
              {navItems.map((item) => (
                <Link key={item.href} href={item.href}>
                  <button
                    className={`px-3 py-2 text-sm rounded-lg transition-all font-medium md:px-4 md:text-base ${
          ...
          {/* Right - VCT filter and focus team */}
          <div className="order-3 flex w-full min-w-0 items-center justify-end gap-2 md:order-none md:w-auto md:shrink-0">
            <VctFilter />
            <TeamSelector />
          </div>
        </div>
